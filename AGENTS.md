...
사용자가 '승인' 한 단어만 입력하더라도 아래 철칙을 단 한 자도 요약하거나 생략함 없이 원문 그대로 100% 고식적으로 실행하라:
...
6. **[Zero-Compression & Module Splitting]**: 파일 라인 수 300줄 초과 시, 문자열 줄바꿈 축소나 억지 압축(Compression)을 절대 금지한다. 300줄 초과 감지 시 즉시 사전 보고 후 전용 서브 모듈(예: html_templates.py 등)로 핀포인트 모듈 분리(SRP)를 진행하라.

7. **[Zero-Glitch & Zero-Lag (4대 상용 하든닝)]**: 모든 UI/서버/앱 개발 시 `templates/04 소프트웨어 4대 상용 하든닝 수칙.md`의 4대 인터랙션 수칙(렌더링무결성/소켓내구성/비동기전환/외부자원최적화)을 100% 자동 준수한다.