# [파일명]: main_c.py (대장 템플릿 - API 웹 서버용) | [원칙] 기승전결 구역 준수, 로직 끝에 12세 수준 요약 주석 필수
...
if __name__=="__main__":
    ...
    # --- [여기서부터 봇 모터 대신 웹 서버 모터(Uvicorn)로 교체됨] ---
    _app=globals().get('app') # [수정] 봇(bot) 대신 웹 서버 앱(app)이 선언되어 있는지 찾습니다.
    if _app:
        print("\n🚀 3. 테스트 완료! 이 프로그램은 [웹 서버]로 판단되어 24시간 상시 대기 모드로 전환합니다.")
        import uvicorn # [수정] Uvicorn 엔진을 가져옵니다.
        print("  -> Uvicorn 웹 서버(FastAPI 등) 루프 시작"); uvicorn.run(_app, host="0.0.0.0", port=8000) # [수정] 무한 루프 모터를 가동합니다.
    else: print("\n✅ 모든 모듈 테스트 완료! 웹 서버(app) 인스턴스가 없으므로 안전하게 종료합니다.")